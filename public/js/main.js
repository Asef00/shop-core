...
(function($) {
    ...
    var totalPriceVariant = function() {
        $(".tf-product-info-list,.tf-cart-item").each(function() {
            ...
            var basePrice = parseFloat(productItem.find(".price-on-sale").data("base-price")) || parseFloat(productItem.find(".price-on-sale").text().replace("تومان", ""));
            ...
            productItem.find(".color-btn, .size-btn").on("click", function() {
                ...
                productItem.find(".price-on-sale").text(
                    `${newPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} تومان`
                );
            ...
            function updateTotalPrice(price, scope) {
                var currentPrice = price || parseFloat(scope.find(".price-on-sale").text().replace("تومان", ""));
                var quantity = parseInt(scope.find(".quantity-product").val());
                var totalPrice = currentPrice * quantity;
                scope.find(".total-price").text(
                    `${totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} تومان`
                );
            }
        });
    };
  ...
  ------------------------------------------------------------------------------------- */
  var FixRTLTemplate = () => {
    document.querySelectorAll('.marquee-wrapper').forEach((i) => {
        i.parentElement.classList.add('fa-slider-rtl');
    })
  }
    ...
    /* RTL | این کد به دلیل اینکه قالب به صورت پیشفرض فارسی سازی شده است ، نیازی نیست و کامنت شده است
    ------------------------------------------------------------------------------------- */
    // var RTL = function() {
