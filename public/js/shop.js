  ...
  var filterProducts = function () {
    ...
    function updateMetaFilter() {
      ...
      if (filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
        appliedFilters.append(
          `<span class="filter-tag">${filters.minPrice} تومان  - ${filters.maxPrice} تومان <span class="remove-tag icon-close" data-filter="price"></span></span>`
        );
      }
      if (filters.color) {
    ...
    function applyFilters() {
      ...
      $("#product-count-grid").html(
        `<span class="count">${visibleProductCountGrid}</span> محصولات یافت شد`
      );
      $("#product-count-list").html(
        `<span class="count">${visibleProductCountList}</span> محصولات یافت شد`
      );
